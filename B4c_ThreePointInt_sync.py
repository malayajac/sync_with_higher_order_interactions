# B4c_ThreePointInt_sync.py
#------------------------------------------------------------
# How to run this code:
# python B4c_hysteresis_sweep.py <network_index> <ic_omega> <ic_theta> <kk2>
#------------------------------------------------------------
import numpy as np
...
ThisDir = "B4c_OtherNets_sync" #CHANGE THIS BY HAND!!!
...
EdgeListDir = "../samplesx5_pentap07nu5"

network_index = int(sys.argv[1])
Net_FileName = f"{EdgeListDir}/penta_p07_nu5_s{network_index}.dat"
print(f"Network input file: {Net_FileName}")
print("#----------------------------------------")
#----------------------------------------
G = nx.read_weighted_edgelist(Net_FileName, nodetype=str, create_using=nx.Graph)
# #----------------------------------------
...
K1_List = np.linspace(-2.0, 2.0, 41)
...
print("#----------------------------------------")
#----------------------------------------
# Output file for order parameter data.
OutFileName = f"{OrdParamDir}/{ThisDir}_r_data_net{network_index}_omega{ic_omega}_theta{ic_theta}_K2_{kk2}.csv"
OutFile = open(OutFileName, "w")
OutFile.write("K1,K2,TimeAvg,StdDev,StdErr,Width\n")
...
for kk1, K1 in enumerate(K1_List):
	...
	fig = plt.figure()
	ax = fig.add_axes([0, 0, 2, 1], xlim=(t_start, t_stop), ylim=(0, 1.1))

	ax.set_title(f"OmegaIC#{ic_omega}, ThetaIC#{ic_theta},\nK1={K1}, K2={K2}")

	ax.plot(t_List, r_df.values, "C0,-")
	ax.plot(t_List[-N_r:], r_df[-N_r:],  "C1,-")
	ax.set_xlabel("t", fontsize="20")
	ax.set_ylabel("r", fontsize="20")
	ax.grid(True)

	FigName = f"{PlotDir}/{ThisDir}_OrderParamVsTime_net{network_index}_omega{ic_omega}_theta{ic_theta}_K1_{kk1}_K2_{kk2}.png"
