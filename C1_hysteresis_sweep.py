# C1_hysteresis_sweep.py
#------------------------------------------------------------
# This is code to see if there is a hysteresis loop.
# Forward sweep, then backward sweep, then forward sweep again.
# The IC of each subsequent trajectory simulation is the last state of the previous trajectory simulation.
#------------------------------------------------------------
# How to run this code:
# python B3_hysteresis_sweep.py <ic_omega> <ic_theta> <kk2>
#------------------------------------------------------------
import numpy as np
...
ThisDir = "C1_hysteresis" #CHANGE THIS BY HAND!!!
...
print("#----------------------------------------")
#----------------------------------------
# Second Forward sweep:
OutFileName_forward2 = f"{OrdParamDir}/{ThisDir}_forward2_r_data_omega{ic_omega}_theta{ic_theta}_K2_{kk2}.csv"
OutFile_forward2 = open(OutFileName_forward2, "w")
OutFile_forward2.write("K1,K2,TimeAvg,StdDev,StdErr,Width\n")
...
for kk1, K1 in enumerate(K1_List):
	...
	# Print sol to file:
	sol_FileName_npy = f"{SolDir}/{ThisDir}_forward2_sol_omega{ic_omega}_theta{ic_theta}_K1_{kk1}_K2_{kk2}.npy"
	np.save(sol_FileName_npy, sol_df.values)
	#----------------------------------------
	# Calculate order parameter:
	x_pos_df = np.cos(sol_df)
	...
	OutFile_forward2.write("{},{},{},{},{},{}\n".format(K1, K2, r_TimeAvg, r_std, r_std_error, r_MaxMin))
	print("{}\t{}\t{:.4e}\t{:.4e}\t{:.4e}\t{:.4e}".format(K1, K2, r_TimeAvg, r_std, r_std_error, r_MaxMin))
 	#----------------------------------------
 	# Use last state of the system as IC for next calculation:
	theta_List0 = sol_df.values[-1]

OutFile_forward2.close()
#------------------------------------------------------------
print("#----------------------------------------")
toc = time()
